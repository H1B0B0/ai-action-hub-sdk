...
class AIActionHub {
        ...
        for (const action of actions) {
            ...
            for (const [paramName, schema] of Object.entries(action.parameters)) {
                if (schema.example !== undefined)
                    exampleParams[paramName] = schema.example;
                else if (schema.type === "string")
                    exampleParams[paramName] = `example_${paramName}`;
                else if (schema.type === "number")
                    exampleParams[paramName] = 0;
    ...
    async handleActionExecution(actionName, params, cors) {
    ...
    buildIntentPrefillPattern(action) {
    ...
    buildIntentPage(action, prefill) {
        const baseUrl = this.baseUrl();
...
<body>
  ...
  <script>
    ...
    async reportToPlatform(actionName, params, result) {
        if (!this.config.apiKey)
            return;
        try {
...
exports.AIActionHub = AIActionHub;
// ─── Framework Helpers ─────────────────────────────────────────────────────
/**
 ...
 */
function createNextHandler(hub) {
 ...
 */
function createExpressMiddleware(hub) {
